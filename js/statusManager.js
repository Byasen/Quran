...
async function saveStateToFirebase() {
  const projectCode = prompt("Enter a project code to SAVE your work:");

  if (!projectCode) {                      // user pressed Cancel
    return;
  }

  try {
    const jsonString = saveState();        // <= your existing exporter (string)
    const jsonObject = JSON.parse(jsonString);

    await firebase
      .database()
      .ref(`projects/${projectCode}/state`)
      .set(jsonObject);                    // overwrite (or create) the doc
...
/**
 * Ask the user for a project code, fetch
 *  projects/<projectCode>/state, and hand it to loadState().
 * Falls back to the random‑verse routine if nothing is found.
 */
async function loadStateFromFirebase() {
  const projectCode = prompt("Enter a project code to LOAD:");

  if (!projectCode) {                       // user pressed Cancel
    alert("Load cancelled – no project code supplied.");
    return;
  }
  ...
  try {
    ...
    if (snap.exists()) {
      const jsonString = JSON.stringify(snap.val()); // loadState expects a string
